...
{
    public class ItemInfoDataEditorWindowView : TableView<ItemInfoData>
    {
        ...
        public ItemInfoDataEditorWindowView()
        {
            TableController = new(this.Q<VisualElement>("left-panel"), nameof(ItemInfoData))
            {
                ...
                BindItem = (elem, index) =>
                {
                    ...
                    var property = So.FindProperty($"infos.Array.data[{index}]");
                    e.BindProperty(property);
                },
                AddItemToSource = () => So.AddArrayElement("infos", p =>
                {
                    p.FindPropertyRelative(FieldName.id).intValue = So.FindProperty("infos").arraySize - 1;
                }),
        ...
        {
            (Asset.Infos as List<ItemInfo>).Sort((x, y) => x.Id.CompareTo(y.Id));
            UpdateView();
        }
