using ElfSoft.Framework;
using System;
using UnityEngine;

namespace ElfSoft.InventorySystem
{
    [Serializable]
    public class ItemSlot
    {
        [SerializeField] protected Item item;
        [SerializeField] protected int stack;
        public Item Item => item;
        public int Stack => stack;


        /// <summary>
        /// 设置物品
        /// </summary>
        public void Set(Item item, int stack)
        {
            if (item != null && stack > 0)
            {
                ChangeField(item, stack);
            }
            else Clear();
        }

        /// <summary>
        /// 清空物品栏
        /// </summary>
        public void Clear() => ChangeField(null, 0);

        /// <summary>
        /// 更改物品数据
        /// </summary>
        protected virtual void ChangeField(Item item, int stack)
        {
            var prviousItem = this.item;
            var previousStack = this.stack;
            this.item = item;
            this.stack = stack;
            EventHub.SendEvent<SlotChangedEventData>(a => a.Init(this, prviousItem, previousStack, item, stack));
        }

        //public bool Swap(Slot slot)
        //{
        //    if (!CheckAllowItem(slot.Item) || !slot.CheckAllowItem(Item)) return false;
        //    if (Item == null || slot.Item == null || Item != slot.Item) return Switch();
        //    int amount = GetCapacity(slot.Item, slot.stack);
        //    Set(slot.Item, stack + amount);
        //    slot.Set(slot.Item, slot.stack - amount);
        //    return true;

        //    bool Switch()
        //    {
        //        var item = Item;
        //        var amount = stack;
        //        Set(slot.Item, slot.stack);
        //        slot.Set(item, amount);
        //        return true;
        //    }
        //}

        /// <summary>
        /// 传入[增加数量],返回实际可增加数
        /// </summary>
        public int GetCapacity(Item item, int toAdd)
        {
            var canAdd = 0;
            if (Allow(item))
            {
                canAdd = item.Info.MaxStack - Stack;
                canAdd = canAdd <= toAdd ? canAdd : toAdd;
            }
            return canAdd;
        }

        /// <summary>
        /// 判断插槽是否可以放入对应物品
        /// </summary>
        public virtual bool Allow(Item item)
        {
            return Item == null || Item.Id == item.Id;
        }

    }
}
